...
def render_geometry_viewer(scene: dict[str, Any]) -> str:
    data = json.dumps(scene, sort_keys=True, ensure_ascii=False).replace("</", "<\\/")
...
const canvas=document.getElementById("gl");canvas.getContext("webgl");let mode="solid",rx=-0.6,ry=0.7,zoom=2.4,drag=false,last=[0,0];
